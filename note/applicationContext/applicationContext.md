...
我们可以发现一个ApplicationContext实现和继承了很多接口和抽象类。通过这些接口和抽象类，来知道ApplicationContext包含了哪些功能，以及分模块的去了解ApplicationContext的实现细节。
...
ApplicationContext 实现了 MessageSource 这个接口来提供 I18N 功能。

MessageSource有以下三个常用方法。
...
spring提供了ResourceBundleMessageSource和StaticMessageSource这两个实现。
...
总的来说就是java标准库的java.net.URL类,虽然对不同协议头的资源做了统一封装。但是在易用性和完整性，例如：
**URL没有classpath和servletContext资源这两种协议**。
spring并不是很满意，所以就做了这样一个封装。

##### ResourceLoader

为了支持上述统一资源获取，Spring还定义了ResourceLoader这个接口，通过ResourceLoader的实现我们可以传入不同协议（包括classpath和servletContext）的路径来获取资源文件

ApplicationContext 实现了 ResourceLoader 这个接口。通过这个接口的方法。

详情: [Resource和ResourceLoader](/note/applicationContext/Resource和ResourceLoader.md)
...
BeanFactory详情: [BeanFactory](/note/applicationContext/BeanFactory.md)
